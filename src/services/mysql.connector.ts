...
const initializeMySqlConnector = () => {
  try {
    // Initialize a pool
    // This is the amount of people that can connect to the database at once
    pool = createPool({
      connectionLimit:
        parseInt(process.env.MY_SQL_DB_CONNECTION_LIMIT != undefined 
          ? process.env.MY_SQL_DB_CONNECTION_LIMIT 
          : ""),
      port:
        parseInt(process.env.MY_SQL_DB_PORT != undefined 
